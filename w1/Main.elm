...
howHotIsThePepper heatUnits =
    if heatUnits < 3 then
        "not hot"
    else if heatUnits > 3 && heatUnits <= 100 then
        "mild"
    else if heatUnits > 100 && heatUnits <= 3000 then
        "medium"
    else 
        "hot"
...
addOne inputList =
    ...
    List.map (\list_element -> list_element + 1) inputList

removeOs : List String -> List String
removeOs inputList =
    -- TODO: remove all entries that start with "O"
    (List.filter (\list_element -> (not (String.contains "O" list_element)) && (not (String.contains "o" list_element))) inputList) 
...
ageDifference person1 person2 =
    Basics.abs (person1.age - person2.age)
...
combinedYears people =
    -- TODO: fix me
    -- List.foldl
    --     (\element sum -> sum + element.age)
    --     0
    --     people
    List.sum (List.map (\item -> item.age) people)
...
pigLatin word =
    ...
    String.toUpper(String.slice 1 2 word) ++ String.slice 1 ((String.length word) - 1) (String.right ((String.length word) - 1) word) ++ "-" ++ String.toLower(String.left 1 word) ++ "ay"
